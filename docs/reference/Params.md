...
- `TESTO_SPICE_MULTIPLE_CLIENTS` - controls the `spice_multiple_clients` VM runtime setting. The built-in default is `no`; the interpreter enables the flag only for the exact value `yes`. It does not change the generated libvirt SPICE XML.
- `TESTO_IMAGE_THRESHOLD` - image-match threshold for ordinary `img` selectors. The built-in default is `0.950000`; accepted values are in the range `0..1`. It is applied at runtime and does not participate in the test cache checksum. `imgtag` selectors use the per-needle `match` value instead.
...
- `TESTO_SNAPSHOT_DEFAULT_POLICY` - default value of a `snapshots` attribute in a [tests declaration](Tests.md#tests-without-hypervisor-snapshots). The built-in default is `always`.
