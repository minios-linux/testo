...
std::string qemu_uri(bool user_mode) {
    return qemu_domain_uri(user_mode);
}
...
void remove_domain(vir::Connect& conn, const std::string& name) {
    ...
    for (auto& snap: domain.snapshots({VIR_DOMAIN_SNAPSHOT_LIST_ROOTS})) {
...
struct ForceFileBackup {
    fs::path destination;
    fs::path backup;
    bool existed = false;
};

struct ForceSnapshotBackup {
    std::string name;
    std::string parent;
    std::string xml;
};

struct ForceDomainBackup {
    std::string name;
    std::string xml;
    std::vector<ForceSnapshotBackup> snapshots;
...
bool domain_is_persistent(const vir::Domain& domain) {
    int result = virDomainIsPersistent(domain.handle);
    if (result < 0) throw std::runtime_error("Can't query domain persistence");
    return result != 0;
}

bool domain_autostart(const vir::Domain& domain) {
    int value = 0;
    if (virDomainGetAutostart(domain.handle, &value) < 0) {
        throw std::runtime_error("Can't query domain autostart");
    }
    return value != 0;
}

void set_domain_autostart(vir::Domain& domain, bool enabled) {
    if (virDomainSetAutostart(domain.handle, enabled ? 1 : 0) < 0) {
        throw std::runtime_error("Can't restore domain autostart");
    }
}

bool network_autostart(const vir::Network& network) {
    int value = 0;
    if (virNetworkGetAutostart(network.handle, &value) < 0) {
        throw std::runtime_error("Can't query network autostart");
    }
    return value != 0;
}

std::string xml_text(const pugi::xml_document& doc) {
    return vir::node_to_string(doc);
}
std::vector<size_t> force_snapshot_restore_order(const std::vector<ForceSnapshotBackup>& snapshots) {
    ...
    while (remaining) {
        ...
        for (size_t i = 0; i < snapshots.size(); ++i) {
            ...
            if (resolved.count(item.name)) continue;
            if (!item.parent.empty() && !resolved.count(item.parent)) continue;
            resolved.insert(item.name);
            order.push_back(i);
            --remaining;
            progress = true;
        }
        if (!progress) throw std::runtime_error("Existing domain snapshot graph is inconsistent");
    }
    return order;
}

ForceDomainBackup backup_domain_metadata(vir::Connect& conn, const std::string& name,
                                         const fs::path& backup_root) {
    auto domain = conn.domain_lookup_by_name(name);
    ForceDomainBackup result;
    result.name = name;
    result.persistent = domain_is_persistent(domain);
    result.active = domain.is_active();
    result.autostart = result.persistent ? domain_autostart(domain) : false;
    result.xml = xml_text(domain.dump_xml(result.persistent
        ? std::initializer_list<virDomainXMLFlags>{VIR_DOMAIN_XML_INACTIVE}
        : std::initializer_list<virDomainXMLFlags>{}));
    result.save_path = backup_root / "domains" / (name + ".save");

    for (auto& snapshot: domain.snapshots()) {
        auto doc = snapshot.dump_xml();
        ForceSnapshotBackup item;
        item.name = doc.child("domainsnapshot").child("name").text().as_string();
        item.parent = doc.child("domainsnapshot").child("parent").child("name").text().as_string();
        item.xml = xml_text(doc);
        result.snapshots.push_back(std::move(item));
    }
    force_snapshot_restore_order(result.snapshots);
    return result;
}
ForceNetworkBackup backup_network_metadata(vir::Connect& conn, const std::string& name) {
    auto network = conn.network_lookup_by_name(name);
    ForceNetworkBackup result;
    result.name = name;
    result.persistent = network.is_persistent();
    result.active = network.is_active();
    result.autostart = result.persistent ? network_autostart(network) : false;
    result.xml = xml_text(network.dump_xml());
    return result;
}

void restore_network_backup(vir::Connect& conn, const ForceNetworkBackup& backup) {
    pugi::xml_document doc;
    if (!doc.load_string(backup.xml.c_str())) {
        throw std::runtime_error("Can't parse backed up network XML: " + backup.name);
    }
    if (backup.persistent) {
        auto network = conn.network_define_xml(doc);
        network.set_autostart(backup.autostart);
        if (backup.active) network.start();
        return;
    }
    if (backup.active) {
        auto handle = virNetworkCreateXML(conn.handle, vir::node_to_string(doc).c_str());
        if (!handle) throw std::runtime_error("Can't restore transient network: " + backup.name);
        virNetworkFree(handle);
    }
}

void restore_domain_snapshots(vir::Domain& domain, const ForceDomainBackup& backup) {
    for (size_t index: force_snapshot_restore_order(backup.snapshots)) {
        pugi::xml_document doc;
        if (!doc.load_string(backup.snapshots[index].xml.c_str())) {
            throw std::runtime_error("Can't parse backed up snapshot XML for " + backup.name);
        }
        domain.snapshot_create_xml(doc, {VIR_DOMAIN_SNAPSHOT_CREATE_REDEFINE});
    }
}
void restore_domain_backup(vir::Connect& conn, const ForceDomainBackup& backup) {
    pugi::xml_document doc;
    if (!doc.load_string(backup.xml.c_str())) {
        throw std::runtime_error("Can't parse backed up domain XML: " + backup.name);
    }

    if (backup.active) {
        conn.restore_domain(backup.save_path);
        if (backup.persistent) {
            conn.domain_define_xml(doc);
            auto domain = conn.domain_lookup_by_name(backup.name);
            set_domain_autostart(domain, backup.autostart);
            restore_domain_snapshots(domain, backup);
        } else {
            auto domain = conn.domain_lookup_by_name(backup.name);
            restore_domain_snapshots(domain, backup);
        }
...
void restore_saved_domains_before_replace(vir::Connect& conn,
                                          const std::vector<ForceDomainBackup>& domains) {
    for (const auto& backup: domains) {
        if (!backup.saved) continue;
        try { remove_domain(conn, backup.name); } catch (...) {}
        restore_domain_backup(conn, backup);
    }
}
...
void import_directory(const fs::path& source, bool force, bool user_mode) {
    ...
    const auto network_uri = qemu_network_uri();
    vir::Connect network_conn(virConnectOpen(network_uri.c_str()));
    if (!network_conn.handle) throw std::runtime_error("Can't connect to QEMU network backend for import");
    ...
    std::set<fs::path> planned_destinations;
    ...
    auto plan_copy = [&](const std::string& rel, const fs::path& dst) {
        ...
        if (!planned_destinations.insert(dst).second) {
            throw std::runtime_error("Container maps multiple files to destination: " + dst.generic_string());
        }
    ...
    for (const auto& item: manifest["machines"]) {
        ...
        if (!machine_names.insert(name).second) {
            throw std::runtime_error("Duplicate virtual machine in container: " + name);
        }
    ...
    for (const auto& item: manifest["networks"]) {
        ...
        if (!network_names.insert(id).second) {
            throw std::runtime_error("Duplicate virtual network in container: " + id);
        }
    ...
    std::set<std::string> flash_names;
    for (const auto& item: manifest["flash_drives"]) {
        auto rel = item.at("path").get<std::string>();
        auto src = checked_container_path(source, rel);
        auto filename = src.filename().generic_string();
        if (filename.size() <= 4 || filename.substr(filename.size() - 4) != ".img") {
            throw std::runtime_error("Invalid flash drive image name in container: " + filename);
        }
        auto name = filename.substr(0, filename.size() - 4);
        if (!flash_names.insert(name).second) {
            throw std::runtime_error("Duplicate flash drive in container: " + name);
        }
    ...
    for (const auto& item: manifest["metadata"]["machines"]) {
        ...
        if (!machine_names.count(name)) {
            throw std::runtime_error("Machine metadata references unknown virtual machine: " + name);
        }
        auto src = checked_container_path(source, rel);
    ...
    for (const auto& item: manifest["metadata"]["flash_drives"]) {
        auto rel = item.at("path").get<std::string>();
        auto name = item.at("name").get<std::string>();
        if (!flash_names.count(name)) {
            throw std::runtime_error("Flash drive metadata references unknown flash drive: " + name);
        }
        auto src = checked_container_path(source, rel);
    ...
    for (const auto& item: manifest["machine_snapshots"]) {
        ...
        pugi::xml_document doc;
        if (!doc.load_file(path.generic_string().c_str())) {
            throw std::runtime_error("Invalid snapshot XML: " + path.generic_string());
        }
        auto machine = item.at("name").get<std::string>();
        if (!machine_names.count(machine)) {
            throw std::runtime_error("Snapshot references unknown virtual machine: " + machine);
        }
        auto name = std::string(doc.child("domainsnapshot").child("name").text().as_string());
        if (name.empty()) {
            throw std::runtime_error("Snapshot XML has no name: " + path.generic_string());
        }
        snapshots.push_back({machine, name,
            doc.child("domainsnapshot").child("parent").child("name").text().as_string(), path});
    }

    std::vector<size_t> snapshot_restore_order;
    std::set<std::pair<std::string, std::string>> resolved_snapshots;
    size_t unresolved_snapshots = snapshots.size();
    while (unresolved_snapshots) {
        bool progress = false;
        for (size_t i = 0; i < snapshots.size(); ++i) {
            const auto& item = snapshots[i];
            auto key = std::make_pair(item.machine, item.name);
            if (resolved_snapshots.count(key)) continue;
            if (!item.parent.empty() && !resolved_snapshots.count({item.machine, item.parent})) continue;
            resolved_snapshots.insert(key);
            snapshot_restore_order.push_back(i);
    ...
    // All file, libvirt and snapshot-graph checks have now passed. No state was modified above.
    std::unique_ptr<TempDirectory> force_backup_dir;
    std::vector<ForceFileBackup> force_file_backups;
    std::vector<ForceDomainBackup> force_domain_backups;
    std::vector<ForceNetworkBackup> force_network_backups;
    bool force_destructive_started = false;

    if (force) {
        force_backup_dir = std::make_unique<TempDirectory>("import-force-backup");
        for (const auto& name: machine_names) {
            if (domain_exists(conn, name)) {
                force_domain_backups.push_back(
                    backup_domain_metadata(conn, name, force_backup_dir->path));
            }
        }
        for (const auto& name: network_names) {
            if (network_exists(network_conn, name)) {
                force_network_backups.push_back(backup_network_metadata(network_conn, name));
            }
        }

        for (size_t i = 0; i < copies.size(); ++i) {
            const auto& plan = copies[i];
            ForceFileBackup backup;
            backup.destination = plan.dst;
            backup.existed = fs::exists(plan.dst);
            if (backup.existed && !fs::is_regular_file(plan.dst)) {
                throw std::runtime_error("Destination is not a regular file: " + plan.dst.generic_string());
            }
            backup.backup = force_backup_dir->path / "files" / std::to_string(i);
            force_file_backups.push_back(std::move(backup));
        }
        try {
            for (auto& backup: force_domain_backups) {
                if (!backup.active) continue;
                ensure_parent(backup.save_path);
                auto domain = conn.domain_lookup_by_name(backup.name);
                domain.save(backup.save_path);
                backup.saved = true;
            }
            for (const auto& backup: force_file_backups) {
                if (!backup.existed) continue;
                copy_file_checked(backup.destination, backup.backup);
            }
        } catch (...) {
            try {
                restore_saved_domains_before_replace(conn, force_domain_backups);
            } catch (const std::exception& rollback_error) {
                std::cerr << "Couldn't restore VM state after force-import backup failure: "
                          << rollback_error.what() << std::endl;
            }
            throw;
        }
        force_destructive_started = true;
    }
    ...
    try {
        ...
        for (const auto& plan: copies) {
            if (plan.identical && !force) continue;
            if (!force) created_files.push_back(plan.dst);
            auto parent = plan.dst.parent_path();
            while (!parent.empty() && !fs::exists(parent)) {
                created_directories.insert(parent);
        ...
        for (size_t index: snapshot_restore_order) {
        ...
        if (force && force_destructive_started) {
            for (const auto& name: machine_names) {
                try { remove_domain(conn, name); } catch (...) {}
            }
            for (const auto& name: network_names) {
                try { remove_network(network_conn, name); } catch (...) {}
            }

            for (auto it = force_file_backups.rbegin(); it != force_file_backups.rend(); ++it) {
                try {
                    if (it->existed) {
                        copy_file_checked(it->backup, it->destination, true);
                    } else if (fs::exists(it->destination)) {
                        fs::remove(it->destination);
                    }
                } catch (const std::exception& rollback_error) {
                    std::cerr << "Couldn't restore force-import file " << it->destination
                              << ": " << rollback_error.what() << std::endl;
                }
            }
            std::vector<fs::path> force_rollback_directories(created_directories.begin(), created_directories.end());
            std::sort(force_rollback_directories.begin(), force_rollback_directories.end(), [](const fs::path& a, const fs::path& b) {
                return std::distance(a.begin(), a.end()) > std::distance(b.begin(), b.end());
            });
            for (const auto& directory: force_rollback_directories) {
                try {
                    if (fs::is_directory(directory) && fs::is_empty(directory)) fs::remove(directory);
                } catch (...) {}
            }
            try { conn.storage_pool_lookup_by_name("testo-storage-pool").refresh(); } catch (...) {}
            try { conn.storage_pool_lookup_by_name("testo-flash-drives-pool").refresh(); } catch (...) {}

            for (const auto& backup: force_network_backups) {
                try {
                    restore_network_backup(network_conn, backup);
                } catch (const std::exception& rollback_error) {
                    std::cerr << "Couldn't restore force-import network " << backup.name
                              << ": " << rollback_error.what() << std::endl;
                }
            }
            for (const auto& backup: force_domain_backups) {
                try {
                    restore_domain_backup(conn, backup);
                } catch (const std::exception& rollback_error) {
                    std::cerr << "Couldn't restore force-import virtual machine " << backup.name
                              << ": " << rollback_error.what() << std::endl;
                }
            }
        } else if (!force) {
            // A normal import starts from an empty namespace, so every domain,
            // network and non-identical file below belongs to this invocation.
            // Roll those changes back before propagating the original error.
            for (const auto& name: machine_names) {
                try { remove_domain(conn, name); } catch (...) {}
            }
            for (const auto& name: network_names) {
            ...
            std::vector<fs::path> rollback_directories(created_directories.begin(), created_directories.end());
            std::sort(rollback_directories.begin(), rollback_directories.end(), [](const fs::path& a, const fs::path& b) {
                return std::distance(a.begin(), a.end()) > std::distance(b.begin(), b.end());
            });
            for (const auto& directory: rollback_directories) {
                try {
                    if (fs::is_directory(directory) && fs::is_empty(directory)) fs::remove(directory);
                } catch (...) {}
            }
